...
import { get_search_products, get_brand_based_products, check_Image } from '@/libs/api';
...
export default function SearchCom({searchRoute='',title, type}) {
    ...
    async function getSearchProducts(){
      ...
      let res = await get_search_products(datas);
      setLoader(false)
      setPageLoader(false)
      if (res && res.message && res.message.length != 0) {
        if (page_no == 1) {
          ...
          setProductList(d => d = [...d, ...res.message])
        }
        page_no = page_no + 1
        no_product = false;
      } else {
    ...
    async function getBrandProducts(){
      let datas = {
        "route": 'brand/'+ searchRoute,
        "page_no": page_no,
        "page_len": 25,
        "isMobile": 1,
        "customer": localStorage['customerRefId'],
      }
      let res = await get_brand_based_products(datas);
      setLoader(false)
      setPageLoader(false)
      if (res && res.message && res.message.length != 0) {
        if (page_no == 1) {
