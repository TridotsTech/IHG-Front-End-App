...
const RelatedCategory = dynamic(()=> import('@/components/Builders/RelatedCategory'))
...
console.log(details,"details")
    ...
    useEffect(()=>{
       ...
       if(productDetail && productDetail.item_categories && productDetail.item_categories.category_name){
           breadcrumb.push({'name' : productDetail.item_categories.category_name , route:productDetail.item_categories.route})
       }
...
const DetailPage = ({productDetail,toast,details}) =>{
   ...
   const loadLightGallery = () =>{
    setTimeout(() => {
      ...
      useMemo(()=>{
        if(webSettings){
            let datas = [{ title: 'Ratings & Reviews', content: productDetail['reviews'] }];
                if(webSettings && webSettings.enable_question_and_answers && webSettings.enable_question_and_answers == 1){
                    datas.unshift({ title: 'Questions', content: productDetail['question_answers'] });
                }
                productDetail['return_description'] ? datas.unshift({ title: 'Return Policy', content: productDetail['return_description'] }) : null;
                productDetail['full_description'] ? datas.unshift({ title: 'Product Detail', content: productDetail['full_description'] }) : null;
                (productDetail['highlights'] && productDetail['highlights'].length != 0) ? datas.unshift({ title: 'Highlights', content: productDetail['highlights'] }) : null;
                (productDetail['product_specification'] && productDetail['product_specification'].length != 0) ? datas.unshift({ title: 'Product Specification', content: productDetail['product_specification'] }) : null;
                productDetail['description'] ? datas.unshift({ title: 'Description', content: productDetail['description'] }) : null;
    ...
    const changeVendor = (res, index) => {
        ...
        for (let i = 0; i < data.vendor_price_list.length; i++) {
            if (i == index) {
                ...
                if (data.vendor_price_list[0].variants && data.vendor_price_list[0].variants.length != 0 && data.has_variants == 1) {
                    let val = data.vendor_price_list[0].variants.findIndex((res) => { return res.attribute_id == data.vendor_price_list[0].default_variant.attribute_id })
    ...
    async function addRemovewish(item) {
        if (item.wish_count == 1) {
            let param = { name: item.wish_id, "customer_id": localStorage['customerRefId']  }
    ...
    <>{(data && Object.keys(data).length != 0) && 
     ...
     {<MobileHeader back_btn={true} title={data.item} search={true} share={true} />}
      ...
      {/* {(data && data.breadcrumb &&  data.breadcrumb.length != 0) ? <div className={`md:hidden flex items-center container p-[10px_0_0_0] gap-[7px]`}>
                            ...
                            {(data.images && data.images.length != 0) ? data.images.map((res, index) => {
                                return (
                                    <>
                                     {(res.video_type && (res.video_type == 'Youtube' || res.video_type == 'Vimeo' || res.video_type == 'Other')) ? 
                                        <a key={index}>
                                         <LoadVideo type={'Detail'} res={res} cssClass={'w-full h-[400px]'} index={index} videoLink={videoLink}/> 
                                        </a>
                                       :
                                        <a key={index} href={check_Image(res.detail_image ? res.detail_image : res.image)}>
                                          ...
                                          <span className={`${res.is_primary == 0 ? 'hidden' : ''}`}><Image className={`w-full h-[401px] object-contain ${res.is_primary == 0 ? 'hidden' : ''}`} src={check_Image(res.detail_image || res.image)} height={200} width={300} alt={res.title}  onError={(e) => {e.target.onerror = null;   e.target.src = '/empty-states.png';}} /></span>       
                                        </a>
                                     }
                                    </>
                                )
        ...
        {(data.related_products && data.related_products.length != 0 && (webSettings && webSettings.enable_related_products == 1)) && 
            <>
                <div className='m-[15px_0]'>
                    {/* <h3 className='text-[15px] font-[500] mb-[8px]'>Related Products</h3> */}
                    <ViewAll data={{ title: "Related Products" }} viewAll={false} />
                    ...
                    <ProductBox productList={data.related_products} rowCount={'flex-[0_0_calc(20%_-_8px)]'} />
                </div>
            </>
        }


        {(data.recently_viewed_products && data.recently_viewed_products.length != 0 && (webSettings && webSettings.enable_recently_viewed_products == 1)) && 
            <>
                <div className='m-[15px_0]'>
                    <h3 className='text-[15px] font-[500] mb-[8px]'>Recently viewed Products</h3>
                    <ProductBox productList={data.recently_viewed_products} rowCount={'flex-[0_0_calc(20%_-_8px)]'} scroll_button={true} scroll_id='recently_viewed'/>
                </div>
            </>
        }

        {(data.products_purchased_together && data.products_purchased_together.length != 0) && 
            <>
                <div className='m-[15px_0]'>
                    <h3 className='text-[15px] font-[500] mb-[8px]'>Products Purchased Together</h3>
                    <ProductBox productList={data.products_purchased_together} rowCount={'flex-[0_0_calc(20%_-_8px)]'} scroll_button={true} scroll_id='you_may_like'/>
                </div>
            </>
        }
        ...
        {(data.you_may_like && data.you_may_like.length != 0) && 
            <>
                <div className='m-[15px_0]'>
                    <h3 className='text-[15px] font-[500] mb-[8px]'>You May also like this</h3>
                    <ProductBox productList={data.you_may_like} rowCount={'flex-[0_0_calc(20%_-_8px)]'} scroll_button={true} scroll_id='you_may_like'/>
                </div>
            </>
        }

        {(data.cross_selling_products && data.cross_selling_products.length != 0) && 
            <>
                <div className='m-[15px_0]'>
                    <h3 className='text-[15px] font-[500] mb-[8px]'>Cross Selling Products</h3>
                    <ProductBox productList={data.cross_selling_products} rowCount={'flex-[0_0_calc(20%_-_8px)]'} scroll_button={true} scroll_id='cross_selling_products'/>
                </div>
            </>
        }
        ...
        {(data.best_seller_category && data.best_seller_category.length != 0) && 
            <>
                <div className='m-[15px_0]'>
                    <h3 className='text-[15px] font-[500] mb-[8px]'>Beat Seller Category</h3>
                    <RelatedCategory productList={data.best_seller_category} rowCount={'flex-[0_0_calc(20%_-_8px)]'} scroll_button={true} scroll_id='cross_selling_products'/>
                </div>
            </>
        }
            ...
            <div className='hidden h-[60px] bg-[#fff] flex items-center justify-between sticky bottom-0 z-[99] p-[10px] shadow-[0_0_5px_#ddd]'>
              {(cartItems && cartItems.length > 0) ? <div onClick={()=>{router.push('/tabs/yourcart')}} className='flex items-center gap-[5px]'>
...
const Attributes = ({data, styles, checkVariantInfo,setData}) =>{
    ...
    useEffect(()=>{
        ...
        if(data.product_attributes && data.product_attributes.length != 0){
            data.product_attributes = data.product_attributes.filter(res=>{ return res.options && res.options.length != 0 })
                    ...
                    {/* {attr.options.map((opt,j)=>{
                     return(
                      <> */}
                       ...
                       {attr.control_type == 'Radio Button List' && attr.options &&  attr.options.length > 0 &&
                         attr.options.map((opt,j)=>{
                            return(
                       ...
                       {attr.control_type == 'Color Boxes' && attr.options &&  attr.options.length > 0 &&
                        attr.options.map((opt,j)=>{
                        return(
                       ...
                       {attr.control_type == 'Checkbox List' && attr.options &&  attr.options.length > 0 &&
                        attr.options.map((opt,j)=>{
                        return(
...
const LoadVideo = ({res, videoLink, index , cssClass, type}) =>{
   ...
   return(
    ...
    <iframe className={`${(type && type == 'Detail' && res.is_primary == 0) ? 'hidden' : ''} ${cssClass ? cssClass: ''}`}
    src={videoLink(res.youtube_video_id ? res.youtube_video_id : res.video_link, res.video_type) + (res.is_primary == 1 ?'?autoplay=1&mute=1' : '') }
    id={index}
    // width={res.width}
    // height={res.height}
    frameBorder="2"
    ...
    <>
      {res.video_type == 'Other' ?
       <video controls className={`${(type && type == 'Detail' && res.is_primary == 0)? 'hidden' : ''}  ${cssClass ? cssClass: ''}`}>
           <source src={videoLink(res.youtube_video_id ? res.youtube_video_id : res.video_link, res.video_type)} type="video/mp4" />
       </video>
       :
       <iframe className={`${(type && type == 'Detail' && res.is_primary == 0)? 'hidden' : ''} ${cssClass ? cssClass: ''}`}
       src={videoLink(res.youtube_video_id ? res.youtube_video_id : res.video_link, res.video_type) + (res.is_primary == 1 ?  '?autoplay=1&muted=1' : '')  }
       id={index}
       // width={res.width}
       // height={res.height}
       frameBorder="2"
